	...
	for (int p = 0; p < NUMP; ++p)
	{
		...
		for (int vs = 0; vs < VARSIZE; ++vs)
		{
			...
			/*
				...
				Наверно потому, что если сделать присоединение в том же
				цикле, где потоки создаются.... он присоеденит 1ый поток,
				а других пока нет (их еще нужно создать) и он не создает
				следующий поток, пока не завершит текущий.

				В общем: join-ов идущих под ряд может быть сколь угодно 
				много, но между ними ни каких других действий не должно
				быть, т.к. иначе все остановится и будет ждать окончания 
				запущенных потоков (мысли вслух).
			*/
